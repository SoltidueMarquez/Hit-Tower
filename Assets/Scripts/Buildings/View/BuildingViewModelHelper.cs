...
namespace Buildings
{
    public class BuildingViewModelHelper
    {
        ...
        private BuildingViewModelData m_Data;
        public event Action<BuildingViewModelData> OnSelectDataChanged;

        public BuildingViewModelHelper()
        {
            m_Data = new BuildingViewModelData();
        }

        public void OnBuildingClicked(BuildingView buildingView)
        {
            if (m_Data.currentSelectedBuildingView == buildingView)
            {
                DeselectBuilding();
            }
            else
        ...
        {
            m_Data.currentSelectedBuildingView = buildingView;
            OnSelectDataChanged?.Invoke(m_Data);
        }

        private void DeselectBuilding()
        {
            if (m_Data.currentSelectedBuildingView == null) return;
            m_Data.currentSelectedBuildingView = null;
            OnSelectDataChanged?.Invoke(m_Data);
        }
